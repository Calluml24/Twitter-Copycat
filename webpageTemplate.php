	<!--- THIS IS AN INCLUDE PAGE FOR ALL THE BASIC STRUCTURE, USED ANOTHER SET FOR LOGGED IN AS WELL --->
<?php
include("connect.php");
?>
<!DOCTYPE html>
...
<!---TO GENERATE THE FULL RESPONSIVE NAV BAR --->
<nav class="navbar navbar-expand-sm navbar-light fixed-top">
	...
	<div class="navbar-collapse collapse text-white" id="navbarNav">
		...
		<ul class="nav navbar-nav text-white ml-auto">			
			<li class="nav-item dropdown">
				...
				while($row = mysqli_fetch_assoc($result)){
					...
					echo"<a class='dropdown-item' href='tagOnlyPostsFinal.php?tag=$tagname'>".$tagname."</a>";
			...
			<li class="nav-item dropdown">
				...
				<div class="dropdown-menu" aria-labelledby="navbarDropdown">
				<?php	
				//POPULATES WITH THE LATEST 5 USERS
				...
				while($row = mysqli_fetch_assoc($result)) {
					...
					echo"<a class='dropdown-item' href='profileOtherUserFinal.php?user=$userid'>".$username."</a>";
					echo"<div class='dropdown-divider'></div>";
				}
				?>
				</div>
		...
		<!--- FORM THAT WILL SEARCH FOR ANY TAG --->
		<form class="nav navbar-nav form-inline" action="tagOnlyPostsFinal.php">
			<input class="form-control mr-sm-2" type="text" name="tag" placeholder="Search a tag..." pattern="[a-z]{1,30}" title="tags should not contain numbers or spaces and be lowercase only." required>
      ...
      <!-- Modal body -->
			<div class="modal-body">
				<form action="postconfirm.php" method="POST" enctype="multipart/form-data" align="center">
					...
					<br><br>
					<input class="posttext" type="text" name="tagName" placeholder="enter a tag.. (i.e cricket)" pattern="[a-z]{1,30}" title="tags should not contain numbers or spaces and be lowercase only."/> <!--- reg = no whitespaces, lowercase only, less that 30 --->
