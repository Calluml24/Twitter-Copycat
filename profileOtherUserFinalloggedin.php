...
include("webpageTemplateLoggedin.php");
		...
		while($row2 = mysqli_fetch_assoc($result2)) {
			...
			//PROCESS TAGS ATTACHED TO THE POST
				...
				//GENERATES A CARD-BODY FOR POST INFORMATION, IF STATEMENT TO FIND IF PROFILE PIC IS SET AND USE PLACEHOLDER IF NOT
					...
					if(!empty($profilePath)) {
						echo"<h5 class='card-title'><img class='posterPic' src='$userid/$profilePath'/>&emsp;<a href='profileOtherUserFinalloggedin.php?user=$userid'>".$usernameDB."</a></h5>
						<p class='postTime'><small class='text-muted postTime'>".time_elapsed_string($postTime)."</small></p>";
						echo"</div>";
						echo"<p class='card-text posts'>".$post."</p>";
					} else {
						echo"<h5 class='card-title'><img class='posterPic'src='profile placeholder.png'/><a href='profileOtherUserFinalloggedin.php?user=$userid'>".$usernameDB."</a></h5>
						<p class='postTime'><small class='text-muted postTime'>".time_elapsed_string($postTime)."</small></p>";
						echo"</div>";
						echo"<p class='card-text posts'>".$post."</p>";
					}
